...
const { startsWith } = require("./util/string");
							...
							children: for (const child of module.children) {
								...
								if (childPath) {
									...
									for (const modulePath of module.paths) {
										if (startsWith(childPath, modulePath)) {
									...
									if (!startsWith(request, "../") && !isAbsolute(request)) {
										request = `./${request}`;
									}
									push({
									...
									try {
										...
										for (const imp of imports) {
											try {
												...
												if (startsWith(dependency, "node:")) continue;
	...
	createSnapshot(startTime, files, directories, missing, options, callback) {
		...
		const checkManaged = (path, managedSet) => {
			...
			for (const immutablePath of this.immutablePathsWithSlash) {
				if (startsWith(path, immutablePath)) {
					managedSet.add(path);
					return true;
				}
			}
			...
			for (const managedPath of this.managedPathsWithSlash) {
				if (startsWith(path, managedPath)) {
					...
					if (err) {
						...
						const process = (set, fn) => {
							...
							for (const file of set) {
								if (startsWith(file, path)) captured.add(file);
							}
